...
func _play_opening_exchange() -> void:
	...
	paper.play()
	table_contact.play()
	await _show_cg(preload("res://assets/art/Day3/briefcase-gun.png"), 1.35, false)
	...
	heartbeat.play()
	breathing.play()
	await _show_cg(preload("res://assets/art/Day3/mc-stressing.png"), 1.35, true)
...
func _play_second_exchange() -> void:
	...
	heartbeat.play()
	await _show_cg(preload("res://assets/art/Day3/mc-stressing.png"), 1.1, true)
...
func _show_cg(texture: Texture2D, hold: float, stressed: bool) -> void:
	...
	reveal.tween_property(cg_image, "scale", Vector2.ONE, _duration(hold + 0.25)).set_trans(Tween.TRANS_SINE)
