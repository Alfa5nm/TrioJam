...
SOURCE = BIRD_DIR / "source/birds_animation_v3_rgba_full.png"
SHEET_OUTPUT = BIRD_DIR / "birds_animation_sheet_v3.png"
FRAME_SIZE = 256
GRID_SIZE = 3
FRAME_COUNT = 9
CELL_MARGIN = 3
...
def main() -> None:
    ...
    for index in range(FRAME_COUNT):
        ...
        frame = source.crop(
            (
                column * cell_width + CELL_MARGIN,
                row * cell_height + CELL_MARGIN,
                (column + 1) * cell_width - CELL_MARGIN,
                (row + 1) * cell_height - CELL_MARGIN,
            )
        )
        frame = frame.resize((FRAME_SIZE, FRAME_SIZE), Image.Resampling.LANCZOS)
        frame_path = BIRD_DIR / f"birds_animation_v3_frame_{index + 1:02d}.png"
